...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # print('operator:',repl_str(operator))
        # print('rest:', repl_str(rest))
        if rest is nil and self_evaluating(operator):
            return operator
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        cur_for = formals
        cur_vals = vals
        while cur_for is not nil:
            frame.define(cur_for.first,cur_vals.first)
            cur_for=cur_for.rest
...
def do_and_form(expressions, env):
    ...
    while cur_expr.rest is not nil:
       if scheme_eval(first,env) is False:
           return False
       cur_expr=cur_expr.rest
       first=cur_expr.first
    tmp=scheme_eval(first,env,True)
    if  tmp is not False:
           return tmp
    else:
        return False
